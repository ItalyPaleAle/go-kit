...
func (m *Mux) applyPrefix(pattern string) string {
	...
	if !strings.HasPrefix(rest, "/") {
		...
		if slashIdx >= 0 {
			...
			// A token that contains a dot but no slash looks like a hostname missing its path component (e.g. "example.com" via a group prefix)
			// Prepending a slash would silently register a path match instead of a host match, masking the panic http.ServeMux raises for these patterns
