...
func (s SMTPEmailer) configureTLS(client *stdsmtp.Client) error {
	...
	if !hasStartTLS {
		...
		// auto mode fails closed: silently downgrading to plaintext would send credentials unencrypted
		// Callers that intentionally want no TLS must set tls=none explicitly
