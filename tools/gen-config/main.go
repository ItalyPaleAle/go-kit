...
func main() {
	configStructFlag := flag.String("struct", filepath.Join("pkg", "config", "config.go"), "Path to config struct file or directory containing config.go")
	yamlDest := flag.String("yaml", filepath.Join(".", "config.sample.yaml"), "Destination path for generated sample YAML")
	mdDest := flag.String("md", filepath.Join(".", "config.md"), "Destination path for generated markdown documentation")
	docsDest := flag.String("docs", "", "Optional docs file path to update between config table markers")

	flag.Usage = func() {
		out := flag.CommandLine.Output()
		fmt.Fprintf(out, "Usage: %s [options]\n\n", filepath.Base(os.Args[0]))
		fmt.Fprintln(out, "Generate sample YAML and markdown docs from a config struct.")
		fmt.Fprintln(out, "\nOptions:")
		flag.PrintDefaults()
	}
	...
	if !strings.HasSuffix(*configStructFlag, ".go") {
		configStructFlag = new(filepath.Join(*configStructFlag, "config.go"))
	}

	err := generateFromStruct(*configStructFlag, *yamlDest, *mdDest, *docsDest)
	if err != nil {
		panic(err)
	}
}
...
func generateFromStruct(configStruct string, yamlDest string, mdDest string, docsDest string) error {
	fmt.Fprintf(os.Stderr, "Reading Config struct from '%s'\n", configStruct)
	fmt.Fprintf(os.Stderr, "Writing YAML config sample to '%s'\n", yamlDest)
	outYAML, err := os.Create(yamlDest)
	if err != nil {
		return err
	}
	defer outYAML.Close()

	fmt.Fprintf(os.Stderr, "Writing config table to '%s'\n", mdDest)
	outFileMD, err := os.Create(mdDest)
	if err != nil {
		return err
	}
	...
	err = parseStructsInFile(configStruct)
	if err != nil {
	...
	if docsDest != "" {
		...
		fmt.Fprintf(os.Stderr, "Updating docs in '%s'\n", docsDest)
		readme, err := os.ReadFile(docsDest)
		if err != nil {
			return err
		}
		...
		readmeFile, err := os.Create(docsDest)
		if err != nil {
			return err
		}
